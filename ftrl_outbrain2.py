...
beta = 0.001   # smoothing parameter for adaptive learning rate
...
def data(path, D):
	...
	row = ad_likelihood_dict.get(ad_id, [])
        likelihood=str(row[3])[:6] if row<>[] else '0.193' #0.193 is general mean 
        x.append(abs(hash('ad_likelihood' + '_' + likelihood)) % D)
...
print("ad_likelyhood")
with open(data_path+'ad_likelihood.csv') as infile:
        doc = csv.reader(infile)
        doc.next()
        ad_likelihood_dict = {}
        for ind, row in enumerate(doc):
            ad_id = int(row[0])
            ad_likelihood_dict[ad_id] = row[1:]
            if ind%100000==0:
                print("ad likelihood : ", ind,ad_likelihood_dict[ad_id])
        print(len(ad_likelihood_dict[ad_id]))
    del doc
...
holdout = 20  # use every N training instance for holdout validation
